...
def extract_feature_given_path(path, sess, low_profile=False):
    if path.endswith('.npz'):
        ...
        np.save(os.path.join(path, "mu_sigma.npy"), {"mu": m, "sigma": s})
    return m, s
...
def calculate_fid_given_paths(paths, inception_path, low_profile=False):
    ...
    with tf.Session() as sess:
        ...
        m1, s1 = extract_feature_given_path(paths[0], sess, low_profile=low_profile)
        m2, s2 = extract_feature_given_path(paths[1], sess, low_profile=low_profile)
