...
class ConvolutionGenerator(nn.Module):
    def __init__(self, upsample=3, map_size=4, out_dim=3):
        """
        ...
        self.fc = nn.Linear(128, map_size * map_size * dims[0])
        ...
        self.visualize = nn.Conv2d(dims[-1], out_dim, 3, padding=1)
        self.tanh = nn.Tanh()
    
    def forward(self, x):
        x = self.relu(self.fc(x)).view(-1, self.dims[0], self.map_size, self.map_size)
        for layers in self.deconvs: x = layers(x)
        x = self.tanh(self.visualize(x))
        return x

class ConvolutionDiscriminator(nn.Module):
    def __init__(self, downsample=3, in_dim=3):
...
class UpsampleGenerator(nn.Module):
    ...
    def forward(self, x):
        x = self.fc(x).view(-1, self.dims[0], self.map_size, self.map_size)
        x = self.relu(self.fc_bn(x))
        for layers in self.deconvs: x = layers(x)
        x = self.tanh(self.visualize(x))
        return x
...
class DownsampleDiscriminator(nn.Module):
    def __init__(self, downsample=3, in_dim=3):
        ...
        for prevDim, curDim in zip(dims[:-1], dims[1:]):
            conv = nn.Sequential()
            conv.conv = nn.Conv2d(prevDim, curDim, 3, padding=1)
            conv.downsample = nn.AvgPool2d(2, 2, ceil_mode=True)
            conv.lrelu = nn.LeakyReLU(0.2, True)
            self.convs.append(conv)
        self.fc = nn.Linear(4 * 4 * dims[-1], 1)
